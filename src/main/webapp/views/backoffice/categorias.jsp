...
<h2 class="mt-2">${titulo}</h2>
	...
	<div class="dropdown show">
		...
		<div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
		    <a class="dropdown-item" href="views/backoffice/mostrar-categorias?categoriaId=-1">Todas</a>
			<c:forEach items="${lista_categorias}" var="c">
				<a class="dropdown-item" href="views/backoffice/mostrar-categorias?categoriaId=${c.id}&categoriaNombre=${c.nombre}">${c.nombre}</a>	
			</c:forEach>
		</div>
	</div> 
			...
			<div class="row-card">
				<c:forEach items="${c.preguntas}" var="p">
					<div class="card">
						<div class="card-body">
							<h4>${p.nombre}</h4>
							<a href="views/backoffice/mostrar-categorias?preguntaId=${p.id}">Ver detalles</a>
						</div>
					</div>
				</c:forEach>
			</div>
